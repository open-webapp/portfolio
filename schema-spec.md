...
All types defined in `src/lib/types.ts`. IDs are `string`, generated via `uid(prefix)` (`src/lib/seed.ts`): `prefix + '-' + <7 random base36 chars>`, e.g. `pos-a1b2c3d`. Prefixes used: `acc` (Account), `pos` (Position), `closed` (ClosedPosition), `tx` (Transaction), `snap` (PortfolioSnapshot), `mapping` (SavedCsvMapping), `bal` (BalanceEntry), `expense` (Expense), `budgettx` (BudgetTransaction), `category` (Category).
...
- Expenses no longer owns a separate year field: it uses the same `selectedScope` (`App.tsx`) that Spend uses. Expenses' `<select>` options are `expenseBudgetYears(amountsByYear, now)` (budgeted years only, descending, no "All"); Spend's remain `All` plus transaction-backed concrete years, initially newest transaction year or `All` when empty. `All` is never a valid Expenses scope — a `BudgetPage` effect reconciles it to the current calendar year when the tab is Expenses and the scope is `All`/not budgeted. The Expenses table itself reads/writes a fixed 3-year window derived from `selectedScope` via `expenseTableColumnYears(selectedScope)` (`[selectedScope-1, selectedScope, selectedScope+1]`), not a union-of-all-years column set.
...
| `amount` | `number` | |
| `spendExpenseId?` | `string` | Optional FK → `ExpenseDefinition.id`, driving `effectiveCategoryId`. Definitions are global/year-independent, so resolution is a flat lookup by id — no `tx.date`-scoped year matching. When set AND resolvable against `budgetExpenseDefinitions`, the linked definition's `categoryId` is the EFFECTIVE category for every actual-spend surface (via `effectiveCategoryId(tx, budgetExpenseDefinitions)` in `src/lib/selectors.ts`) — `categoryId` above is only a fallback, used when `spendExpenseId` is unset or unresolvable (e.g. the definition was deleted). Set via the `SpendCategoryPicker` component (Spend records per-cell edit, bulk-edit action bar, Add Record form) — the picker lists ALL `budgetExpenseDefinitions` regardless of the transaction's or page's year (see `product-behavior.md`'s Budget section for the UX implications of this global-visibility change) — or auto-linked by `propagateSpendLinksByAutoTag` (see below) on import/auto-tag/manual edit. **"Clearing" the link always sets this field to the literal value `undefined`** (an explicit key with value `undefined`, which `JSON.stringify`/the IndexedDB structured-clone path both drop — so a cleared record is stored indistinguishably from one that never had `spendExpenseId` set; there is no separate "deleted" marker) — done via the `SpendCategoryPicker`'s leading `— Uncategorized —` option (per-cell edit, Add Record form). A manually-cleared record can be overwritten again the next time `propagateSpendLinksByAutoTag` runs over its `autoTag` group (see below) |
...
**Dedup on import** (`IMPORT_BUDGET_TRANSACTIONS` / `importBudgetTransactions` in `state.ts`): natural key `date|description|amount|accountName` (normalized `date`, raw values otherwise, no further normalization; neither resolved field is part of the key — two rows differing only by expense link dedup as duplicates), checked against existing `budgetTransactions` and against earlier rows already accepted in the same import batch — duplicates are silently dropped. `spendExpenseId` resolution happens **before** dedup but does not participate in the dedup key: `importBudgetTransactions` runs `applyAutoTags(rows)` on the batch first (fills `autoTags` under the merged combined-5 + cross-dedup rules, user casing wins on clash), then resolves each row's `spendExpenseId` via a per-`autoTag` lookup against carriers already in `existing` plus the current batch (most-common linked `spendExpenseId` wins, ties → first in `existing` order; unresolved or dangling → "Other" `categoryId` with `spendExpenseId` unset) — see `propagateSpendLinksByAutoTag` below for the shared election rule. `tags` (already-validated `string[]` user-tag passthrough from the CSV layer) are carried into `toAdd` unchanged and likewise never participate in the dedup key — two rows differing only by `tags` dedup as duplicates, first row's tags win. The resolve+dedup logic is factored into the pure `resolveBudgetImportRows(existing, rows, categories, budgetExpenseDefinitions): { toAdd, duplicateCount }`, which `importBudgetTransactions` calls internally and which `BudgetPage` also calls directly (pre-dispatch) to report imported/skipped counts in the import dialog.

## Category

**Not part of `AppState`'s persistence envelope.** Categories live in the standalone Global Category Store.
...
| `deletedAt?` | `string` | ISO timestamp tombstone. `DELETE_CATEGORY` sets `deletedAt` (+ refreshes `updatedAt`); record is retained, never physically removed. Manage Categories page (`#/categories`, reached via Settings) is the only delete UI. |
| `excludeFromSpend?` | `boolean` | Default absent (`undefined`/falsy) = included/not excluded. When `true`, Budget page treats every transaction in this category as excluded from all actual-spend-derived figures app-wide (Spend tab KPI tiles) and Spend records hides its rows by default. Budgeted/planned figures — including the Expenses tab's `plannedSpendSummary`/`plannedSavingsRate`/`plannedFrequencySplit`/`planChanges` stat cards — are never affected: those always count every non-Income spend line regardless of `excludeFromSpend`. Category `<select>` controls elsewhere in the app are unaffected — excluded categories stay selectable. Set from the Manage Categories page (`#/categories`), dispatching `SET_CATEGORY_EXCLUDE_FROM_SPEND` on the Global Category Store |

An "Other" category is used as the fallback whenever `spendExpenseId` resolution (see `propagateSpendLinksByAutoTag` below) misses. `visibleCategories()` (`src/lib/categoryStore.ts`) filters out tombstoned (`deletedAt`-set) records for every UI list.

## Spend-link propagation (`propagateSpendLinksByAutoTag`)

There is no portfolio-level description→expense mapping table. Instead, `src/lib/state.ts`'s pure `propagateSpendLinksByAutoTag(transactions: BudgetTransaction[], definitions: ExpenseDefinition[]): BudgetTransaction[]` links `spendExpenseId`/`categoryId` by shared `autoTag` string:

- Groups transaction indices by each distinct `autoTag` value present in `t.autoTags`.
- Processes tags in sorted order. For each tag, the most-common `spendExpenseId` among that tag's carriers wins (ties → first carrier in input order); a carrier with no link doesn't vote.
- If the winning `spendExpenseId` resolves to a live `ExpenseDefinition`, every carrier of that tag is set to `{ spendExpenseId: winner, categoryId: definition.categoryId }` — this overwrites existing links, including a manually-cleared ("Uncategorized") one.
- A tag with no linked carrier, or whose winner has no matching definition, is skipped — its carriers are left untouched.
- A transaction with multiple `autoTags` is processed once per tag in sorted order; the last tag whose winner resolves wins (deterministic overwrite).
- Untagged or singleton-tag-with-no-other-carrier transactions are untouched.

**Trigger points**:
- `IMPORT_BUDGET_TRANSACTIONS` / `importBudgetTransactions`: batch rows run `applyAutoTags` first, then `resolveBudgetImportRows` resolves each row's `spendExpenseId` via the same most-common/tie-break rule looking at both `state.budgetTransactions` and the current batch (no full recluster).
- `AUTO_TAG_BUDGET_TRANSACTIONS` / `autoTagBudgetTransactions`: recomputes `autoTags` for all transactions, then runs full propagation over the result.
- `ADD_BUDGET_TRANSACTION` / `UPDATE_BUDGET_TRANSACTION` / `UPDATE_BUDGET_TRANSACTIONS_BULK`: after the base mutation, propagation runs over all `budgetTransactions` — this overwrites every same-`autoTag` sibling, not just unlinked ones. For bulk, the dispatch's explicit `spendExpenseId` patch applies to the selected rows first, then propagation cascades from there.

Manual per-cell/bulk/Add-Record spend pickers (`SpendCategoryPicker`) still write a plain `spendExpenseId` patch directly — the cascade happens afterward in the state layer, not in the component.
...
- `DELETE_EXPENSE_DEFINITION { id: string }`: Remove an `ExpenseDefinition` and its amount entries from every year; no-op if not found. Callers must check `expenseDefinitionInUse(state, id)` first and block/alert if any `BudgetTransaction` (in any year) still references it via `spendExpenseId`.
...
- `IMPORT_BUDGET_TRANSACTIONS { rows: { date, description, amount, accountName?, tags? }[]; categories: Category[] }`: Resolves each row's `spendExpenseId` via the autoTag-based lookup in `resolveBudgetImportRows` (verified against `state.budgetExpenseDefinitions`; miss or dangling winner → "Other" `categoryId`, `spendExpenseId` unset), derives `categoryId` from the linked definition, then appends rows deduped on `date|description|amount|accountName` against existing `budgetTransactions` and earlier rows in the same batch. `tags` pass through unchanged (never in the key). `categories` are supplied by `BudgetPage`; definitions come from `AppState`.
...
**Budget page fields are split across two layers**: `App.tsx` owns `period` (`'expenses' | 'spend' | 'analytics'`, initialized to `'spend'` per app session) and `selectedScope` (`SpendScope = string | typeof SPEND_ALL_YEARS`, init: newest transaction year else `SPEND_ALL_YEARS`) — `selectedScope` is shared by both the Spend and Expenses tabs, passed down as props, and reconciled by two `period`-gated effects in `BudgetPage.tsx` (Spend resets to `SPEND_ALL_YEARS` when invalid; Expenses resets to the current calendar year when `SPEND_ALL_YEARS` or not in `expenseBudgetYears`). `BudgetPage`'s own `recordSearch`/`recSortBy`/`recSortDir`/`recPage`/`editingCell`/`cellDraft`/dialog and form drafts, plus `BudgetExpensesTab`'s own `filterCategoryId`/`sortBy`/`sortDir`/dialog and form drafts, are component-local and not persisted. `budgetExpenseDefinitions`/`budgetExpenseAmountsByYear`/`budgetTransactions` are persisted `AppState` fields; `categories` are global-store props.
...
- **Migration-tolerant field coalescing**: `loadPersistedApp` rebuilds `AppState` from a fixed whitelist against `initialState()` defaults; missing fields default and stale keys — including a legacy `categoryMappings` — are dropped. `budgetExpenseDefinitions`/`budgetTransactions` default to `[]`; `budgetExpenseAmountsByYear` defaults to `{}`. `dropLegacyBudgetIncome` removes retired manual-income fields before coalescing.
...
`ExportableState` (`src/lib/importExport.ts`, the manual backup-download/restore-file format) excludes global `categories`; `buildExportableState`/`decryptImportEnvelope`/`replaceImportedState` do not read or write them. `UnencryptedPortfolioExport` is an alias of `ExportableState` (no additional fields).
