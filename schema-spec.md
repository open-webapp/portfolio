...
| `amount` | `number` | |
| `spendExpenseId?` | `string` | Optional FK → `Expense.id`, driving `effectiveCategoryId`. When set AND resolvable against a same-calendar-year `Expense` (`tx.date.slice(0,4)` matched in `budgetExpensesByYear`), the linked expense's `categoryId` is the EFFECTIVE category for every actual-spend surface (via `effectiveCategoryId(tx, budgetExpensesByYear)` in `src/lib/selectors.ts`) — `categoryId` above is only a fallback, used when `spendExpenseId` is unset or unresolvable. Set via the `SpendCategoryPicker` component (Spend records per-cell edit, bulk-edit action bar, Add Record form) or auto-linked on import (first same-year `Expense` whose `categoryId` matches the resolved `categoryId`, array order). **"Clearing" the link always sets this field to the literal value `undefined`** (an explicit key with value `undefined`, which `JSON.stringify`/the IndexedDB structured-clone path both drop — so a cleared record is stored indistinguishably from one that never had `spendExpenseId` set; there is no separate "deleted" marker) — done via the `SpendCategoryPicker`'s leading `— Uncategorized —` option (per-cell edit, Add Record form) or by `reapplyMappingsToTransactions` when a re-resolved mapping's category differs from the transaction's current effective category (see below) |
