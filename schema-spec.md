...
| `tags?` | `string[]` | Free-form per-record tags. Validated `[a-zA-Z0-9]` only, max 10 chars/tag, max 5/record; case-insensitive dedup within the record (first casing kept). Never stored as `[]` — key omitted when empty; clearing sets `tags: undefined` |
