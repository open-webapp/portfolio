...
| `amount` | `number` | |
| `spendExpenseId?` | `string` | FK → `Expense.id`. When set AND resolvable against a same-calendar-year `Expense` (`tx.date.slice(0,4)` matched in `budgetExpensesByYear`), the linked expense's `categoryId` is the EFFECTIVE category for every actual-spend surface (via `effectiveCategoryId(tx, budgetExpensesByYear)` in `src/lib/selectors.ts`) — `categoryId` above is only a fallback, used when `spendExpenseId` is unset or unresolvable. Set via the `SpendCategoryPicker` component (Spend records per-cell edit, bulk-edit action bar, Add Record form) or auto-linked on import (first same-year `Expense` whose `categoryId` matches the resolved `categoryId`, array order) |
