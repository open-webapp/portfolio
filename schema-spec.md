...
- **Seed-from-nearest** (`nearestBudgetExpensesYear`/`nearestBudgetIncomeYear`, consulted by `addBudgetExpense`/`updateBudgetExpense`/`deleteBudgetExpense`/`setBudgetIncome`): selecting/editing a year with no existing entry copies the nearest existing year's data (closest by absolute year distance; ties resolve to the earlier year) into a new entry for that year. The search **skips any candidate year whose stored snapshot is itself empty** (via `isEmptyExpensesSnapshot`/`isEmptyIncomeSnapshot`), expanding outward in both directions until a non-empty year is found; if no non-empty year exists anywhere, the new entry starts empty (`[]` for expenses, `{monthly:0, yearly:0}` for income) — `nearestYearKey` (`src/lib/state.ts`), the shared underlying search, takes this skip-empty predicate as an optional param.
- **Eager snapshot on selected-year switch** (`ensureBudgetExpensesSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear`, dispatched together via `ENSURE_BUDGET_YEAR_SNAPSHOT { year }`): switching to a year with no existing snapshot **persists** a real entry into `budgetExpensesByYear[year]`/`budgetIncomeByYear[year]` immediately, cloned from the nearest existing **non-empty** year (same skip-empty search as above) — no longer a purely virtual/display-only clone. If no non-empty year exists anywhere yet, this is a no-op (nothing persisted, stays virtual) rather than seeding an empty entry — so a year visited before any data existed can still pick up data added afterward to a different year, instead of being permanently stuck empty. This differs from `seedBudgetExpensesForYear`/`seedBudgetIncomeForYear` (used by the mutation helpers above), which always create the entry, seeding empty when there's no nearest non-empty year, since those are about to write into it immediately — that transient empty seed is always overwritten by the caller's mutation before it could ever be persisted standalone. Two independent call sites in `BudgetPage` change `selectedYear` and must both guard-and-dispatch this: the Year dropdown's `onChange` (Yearly mode only), and `handleAddRecord` (adding a spend record dated in a year other than the current selection also switches `selectedYear`).
  - **Invariant**: `budgetExpensesByYear`/`budgetIncomeByYear` never persist an empty snapshot under any code path.
- **Never-empty-on-delete/zero**: `deleteBudgetExpense` removes a year's key entirely (rather than leaving `[]`) if deleting the last expense would leave it empty; `setBudgetIncome` removes a year's key entirely (rather than leaving `{monthly:0, yearly:0}`) if the result would be all-zero — so the year can later re-inherit data via seed-from-nearest/rollover instead of being pinned to a stored-empty value.
- **Rollover on load** (`rolloverBudgetExpensesIfNeeded`/`rolloverBudgetIncomeIfNeeded`, dispatched via `ROLLOVER_BUDGET_EXPENSES_IF_NEEDED`/`ROLLOVER_BUDGET_INCOME_IF_NEEDED` at app load): if the current calendar year has no entry yet but an earlier year does, the nearest prior non-empty year's data is auto-copied forward as the new current year's starting entry. Each also runs `stripEmptyBudgetSnapshots(state)` as its first step — a migration that removes any pre-existing persisted empty snapshot (from before this skip-empty behavior existed) before the normal rollover-seed logic runs; runs on every load, cheap no-op once already clean.
- **Display fallback**: `resolveBudgetExpensesForYear`/`resolveBudgetIncomeForYear` apply the same skip-empty nearest-year search for rendering `BudgetPage`'s selected year — a year's own empty snapshot is treated as absent and the display falls through to the nearest non-empty year — but this is read-only and persists nothing itself.
...
| `amount` | `number` | |
| `spendExpenseId?` | `string` | Optional FK → `Expense.id`, driving `effectiveCategoryId`. When set AND resolvable against a same-calendar-year `Expense` (`tx.date.slice(0,4)` matched in `budgetExpensesByYear`), the linked expense's `categoryId` is the EFFECTIVE category for every actual-spend surface (via `effectiveCategoryId(tx, budgetExpensesByYear)` in `src/lib/selectors.ts`) — `categoryId` above is only a fallback, used when `spendExpenseId` is unset or unresolvable. Set via the `SpendCategoryPicker` component (Spend records per-cell edit, bulk-edit action bar, Add Record form) or auto-linked on import (via shared helper `resolveSpendExpenseForCategory(expensesForYear, categoryId)`, `src/lib/categoryStore.ts` — first same-year `Expense` whose `categoryId` matches the resolved `categoryId`, array order). **"Clearing" the link always sets this field to the literal value `undefined`** (an explicit key with value `undefined`, which `JSON.stringify`/the IndexedDB structured-clone path both drop — so a cleared record is stored indistinguishably from one that never had `spendExpenseId` set; there is no separate "deleted" marker) — done via the `SpendCategoryPicker`'s leading `— Uncategorized —` option (per-cell edit, Add Record form). It is **not** cleared unconditionally by `reapplyMappingsToTransactions` — see that function's "Auto re-apply" note below for its opportunistic-relink behavior, including the accepted tradeoff that a manual "Uncategorized" choice can be silently overwritten |

**`effectiveCategoryId(tx, budgetExpensesByYear)`** (`src/lib/selectors.ts`): resolution rule — prefer the `Expense` at `budgetExpensesByYear[tx.date.slice(0,4)]` whose `id === tx.spendExpenseId` when `spendExpenseId` is set and that lookup resolves; otherwise fall back to `tx.categoryId`. Used everywhere a transaction's "real" category is needed for actual-spend figures, instead of reading `categoryId` directly.
...
**Dedup on import** (`IMPORT_BUDGET_TRANSACTIONS` / `importBudgetTransactions` in `state.ts`): natural key `date|description|amount|accountName` (normalized `date`, raw values otherwise, no further normalization; `categoryId` is **not** part of the key — two rows differing only by category now dedup as duplicates, a breaking change from the prior key), checked against existing `budgetTransactions` and against earlier rows already accepted in the same import batch — duplicates are silently dropped. `categoryId` resolution (via `CategoryMapping` substring match, falling back to "Other") and `spendExpenseId` auto-resolution (first same-year `Expense`, array order, whose `categoryId` matches the resolved `categoryId`; omitted if none matches) both happen **before** dedup, but neither participates in the dedup key. The resolve+dedup logic is factored into the pure `resolveBudgetImportRows(existing, rows, categories, categoryMappings, budgetExpensesByYear): { toAdd, duplicateCount }`, which `importBudgetTransactions` calls internally and which `BudgetPage` also calls directly (pre-dispatch) to report imported/skipped counts in the import dialog.
...
**`resolveSpendExpenseForCategory(expensesForYear: Expense[], categoryId: string): Expense | undefined`** (`src/lib/categoryStore.ts`): shared helper — returns the first array-order `Expense` in `expensesForYear` whose `categoryId` matches, else `undefined`. Two call sites: `resolveBudgetImportRows`/`importBudgetTransactions` (import auto-link, `state.ts`) and `reapplyMappingsToTransactions` (below).

**Auto re-apply**: no manual UI trigger exists — `REAPPLY_CATEGORY_MAPPINGS { categoryMappings }` (main `AppState` reducer, since it mutates `budgetTransactions`) is dispatched automatically, immediately after any mutation of a `CategoryMapping`, from every such call site: Settings > Categories tab's substring add/edit handlers, its CSV/JSON mapping import merge, and BudgetPage's per-cell Spend Category edit and Add Record form (each after their own `UPSERT_CATEGORY_MAPPING`/`categoryDispatch` call). `reapplyMappingsToTransactions(transactions, mappings, budgetExpensesByYear)` (`src/lib/categoryStore.ts`) implements it: for each transaction, resolve `resolved = resolveCategoryIdForDescription(mappings, description)` and compare against `effective = effectiveCategoryId(tx, budgetExpensesByYear)`. `categoryId` is always reassigned to `resolved` when a mapping matches. `spendExpenseId` is auto-linked via `resolveSpendExpenseForCategory(expensesForYear, resolved)` (same-year, same-category `Expense` lookup) whenever EITHER `resolved !== effective` (category changed) OR `spendExpenseId` was already unset (opportunistic case); if no matching `Expense` exists, `spendExpenseId` becomes/stays `undefined`. If `resolved === effective` AND `spendExpenseId` was already set, it's left untouched. Idempotent and non-destructive (only reassigns categories/links via existing mapping rules, never deletes/adds rows) — no confirm dialog needed. **Accepted tradeoff (intentional, not guarded)**: this can silently re-link a transaction the user had previously manually set to "Uncategorized" via the per-cell picker, if a matching `Expense` now exists for that row's year+category.
...
- `ENSURE_BUDGET_YEAR_SNAPSHOT { year: string }`: dispatched from `BudgetPage` whenever `selectedYear` changes to a year with no existing snapshot — from the Year dropdown's `onChange` (Yearly mode only) and from `handleAddRecord` (adding a spend record for a new year) — fans out to both `ensureBudgetExpensesSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear` (see Year-scoped budget storage note above)
...
- `IMPORT_BUDGET_TRANSACTIONS { rows: { date, description, amount, accountName? }[]; categories: Category[]; categoryMappings: CategoryMapping[]; budgetExpensesByYear: Record<string, Expense[]> }`: Resolves each row's `categoryId` via the action's own `categories`/`categoryMappings` (case-insensitive substring match against `description`, latest-`updatedAt` wins among live/non-tombstoned mappings; falls back to "Other"), then auto-resolves `spendExpenseId` from `budgetExpensesByYear` (first same-year `Expense`, array order, whose `categoryId` matches; omitted if none matches), then appends rows deduped on `date|description|amount|accountName` against existing `budgetTransactions` and against earlier rows in the same batch. `categories`/`categoryMappings`/`budgetExpensesByYear` are supplied by the caller (`BudgetPage`, via its global-store props and `state.budgetExpensesByYear`) — this action no longer reads `categories`/`categoryMappings` off `AppState`
- `REAPPLY_CATEGORY_MAPPINGS { categoryMappings: CategoryMapping[] }`: Rewrites `budgetTransactions` via `reapplyMappingsToTransactions` (see `## CategoryMapping`'s "Auto re-apply" note above for the categoryId/spendExpenseId semantics), from the given `categoryMappings` (supplied by the caller — `Settings.tsx` or `BudgetPage.tsx`, both via their global-store `categoryMappings` prop, dispatched automatically right after any mapping mutation, no manual trigger); stays on the main reducer since it mutates `AppState.budgetTransactions`, even though `categoryMappings` themselves live in the Global Category Store
