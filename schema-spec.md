...
- The Expenses tab's Category Breakdown panel and multi-year Expense table read/write across an arbitrary set of years at once (own local `breakdownYear`, plus a union-of-years column set); the Spend tab reads/writes whichever year is selected via its own Year dropdown (`BudgetPage`'s local `selectedYear` state).
...
- **Eager snapshot on selected-year switch** (`ensureExpenseAmountsSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear`, dispatched together via `ENSURE_BUDGET_YEAR_SNAPSHOT { year }`): switching `selectedYear` to a year with no existing snapshot **persists** a real entry into `budgetExpenseAmountsByYear[year]`/`budgetIncomeByYear[year]` immediately, cloned from the nearest existing **non-empty** year (same skip-empty search as above) — not a purely virtual/display-only clone. If no non-empty year exists anywhere yet, this is a no-op (nothing persisted, stays virtual) rather than seeding an empty entry — so a year visited before any data existed can still pick up data added afterward to a different year, instead of being permanently stuck empty. Two call sites in `BudgetPage` change `selectedYear` and must both guard-and-dispatch this: the Spend tab's Year dropdown `onChange`, and `handleAddRecord` (adding a spend record dated in a year other than the current selection also switches `selectedYear`).
...
**Dedup on import** (`IMPORT_BUDGET_TRANSACTIONS` / `importBudgetTransactions` in `state.ts`): natural key `date|description|amount|accountName` (normalized `date`, raw values otherwise, no further normalization; `categoryId` is **not** part of the key — two rows differing only by category now dedup as duplicates, a breaking change from the prior key), checked against existing `budgetTransactions` and against earlier rows already accepted in the same import batch — duplicates are silently dropped. `categoryId` resolution (via `CategoryMapping` substring match, falling back to "Other") and `spendExpenseId` auto-resolution (first `ExpenseDefinition`, array order, whose `categoryId` matches the resolved `categoryId`, no year filter; omitted if none matches) both happen **before** dedup, but neither participates in the dedup key. The resolve+dedup logic is factored into the pure `resolveBudgetImportRows(existing, rows, categories, categoryMappings, budgetExpenseDefinitions): { toAdd, duplicateCount }`, which `importBudgetTransactions` calls internally and which `BudgetPage` also calls directly (pre-dispatch) to report imported/skipped counts in the import dialog.
...
**`resolveSpendExpenseForCategory(definitions: ExpenseDefinition[], categoryId: string): ExpenseDefinition | undefined`** (`src/lib/categoryStore.ts`): shared helper — returns the first array-order `ExpenseDefinition` in `definitions` whose `categoryId` matches, else `undefined`; no year filter, since definitions are global. Two call sites: `resolveBudgetImportRows`/`importBudgetTransactions` (import auto-link, `state.ts`) and `reapplyMappingsToTransactions` (below).

**Auto re-apply**: no manual UI trigger exists — `REAPPLY_CATEGORY_MAPPINGS { categoryMappings }` (main `AppState` reducer, since it mutates `budgetTransactions`) is dispatched automatically, immediately after any mutation of a `CategoryMapping`, from every such call site: Settings > Categories tab's substring add/edit handlers, its CSV/JSON mapping import merge, and BudgetPage's per-cell Spend Category edit and Add Record form (each after their own `UPSERT_CATEGORY_MAPPING`/`categoryDispatch` call). `reapplyMappingsToTransactions(transactions, mappings, budgetExpenseDefinitions)` (`src/lib/categoryStore.ts`) implements it: for each transaction, resolve `resolved = resolveCategoryIdForDescription(mappings, description)` and compare against `effective = effectiveCategoryId(tx, budgetExpenseDefinitions)`. `categoryId` is always reassigned to `resolved` when a mapping matches. `spendExpenseId` is auto-linked via `resolveSpendExpenseForCategory(budgetExpenseDefinitions, resolved)` (same-category `ExpenseDefinition` lookup, global, no year filter) whenever EITHER `resolved !== effective` (category changed) OR `spendExpenseId` was already unset (opportunistic case); if no matching definition exists, `spendExpenseId` becomes/stays `undefined`. If `resolved === effective` AND `spendExpenseId` was already set, it's left untouched. Idempotent and non-destructive (only reassigns categories/links via existing mapping rules, never deletes/adds rows) — no confirm dialog needed. **Accepted tradeoff (intentional, not guarded)**: this can silently re-link a transaction the user had previously manually set to "Uncategorized" via the per-cell picker, if a matching `ExpenseDefinition` now exists for that row's category (global match — no longer scoped to the row's year).
...
- `DELETE_EXPENSE_DEFINITION { id: string }`: Remove an `ExpenseDefinition` and its `amount` entry from every year in `budgetExpenseAmountsByYear`; no-op if not found. Callers must check `expenseDefinitionInUse(state, id)` first and block/alert if any `BudgetTransaction` (in any year) still references it via `spendExpenseId`
...
- `ENSURE_BUDGET_YEAR_SNAPSHOT { year: string }`: dispatched from `BudgetPage` whenever `selectedYear` (Spend tab) changes to a year with no existing snapshot — from the Spend tab's Year dropdown `onChange` and from `handleAddRecord` (adding a spend record for a new year) — fans out to both `ensureExpenseAmountsSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear` (see Budget storage note above)
...
- `IMPORT_BUDGET_TRANSACTIONS { rows: { date, description, amount, accountName? }[]; categories: Category[]; categoryMappings: CategoryMapping[] }`: Resolves each row's `categoryId` via the action's own `categories`/`categoryMappings` (case-insensitive substring match against `description`, latest-`updatedAt` wins among live/non-tombstoned mappings; falls back to "Other"), then auto-resolves `spendExpenseId` from `state.budgetExpenseDefinitions` (first definition, array order, whose `categoryId` matches, no year filter; omitted if none matches), then appends rows deduped on `date|description|amount|accountName` against existing `budgetTransactions` and against earlier rows in the same batch. `categories`/`categoryMappings` are supplied by the caller (`BudgetPage`, via its global-store props) — this action reads `budgetExpenseDefinitions` off `AppState` itself, not from the action payload
