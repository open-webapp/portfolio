...
| `excludeFromSpend?` | `boolean` | Default absent (`undefined`/falsy) = included/not excluded. When `true`, Budget page treats every transaction in this category as excluded from all actual-spend-derived figures app-wide (Spend tab KPI tiles, Expenses table per-row Actual/Variance, Category Breakdown panel — which drops the category's row entirely) and Spend records hides its rows by default. Budgeted/planned figures are never affected. Category `<select>` controls elsewhere in the app are unaffected — excluded categories stay selectable. Set from the Manage Categories page (`#/categories`), dispatching `SET_CATEGORY_EXCLUDE_FROM_SPEND` on the Global Category Store |
