...
- **Migration-tolerant field coalescing**: `loadPersistedApp`/`loadLegacyPlaintextApp` rebuild `AppState` from a fixed whitelist against `initialState()` defaults; missing fields default and stale keys are dropped. `budgetExpenseDefinitions` defaults to `[]`, `budgetExpenseAmountsByYear` to `{}`, and `budgetTransactions` to `[]`. `dropLegacyBudgetIncome` removes retired manual-income fields before coalescing. `coalesceWithDefaults` does not migrate categories; the one-time cross-portfolio seed reads the raw pre-coalesce blob via `loadRawPersistedBlob(key)`.
