...
- **Eager snapshot on selected-year switch** (`ensureBudgetExpensesSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear`, dispatched together via `ENSURE_BUDGET_YEAR_SNAPSHOT { year }`): switching to a year with no existing snapshot **persists** a real entry into `budgetExpensesByYear[year]`/`budgetIncomeByYear[year]` immediately, cloned from the nearest existing year (closest by absolute year distance; ties resolve to the earlier year) — no longer a purely virtual/display-only clone. If no year has data anywhere yet, this is a no-op (nothing persisted, stays virtual) rather than seeding an empty entry — so a year visited before any data existed can still pick up data added afterward to a different year, instead of being permanently stuck empty. This differs from `seedBudgetExpensesForYear`/`seedBudgetIncomeForYear` (used by the mutation helpers above), which always create the entry, seeding empty when there's no nearest year, since those are about to write into it immediately. Two independent call sites in `BudgetPage` change `selectedYear` and must both guard-and-dispatch this: the Year dropdown's `onChange` (Yearly mode only), and `handleAddRecord` (adding a spend record dated in a year other than the current selection also switches `selectedYear`).
