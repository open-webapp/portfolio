...
- `ENSURE_BUDGET_YEAR_SNAPSHOT { year: string }`: dispatched from `BudgetPage`'s Year dropdown `onChange` (Yearly mode only) when switching to a year with no existing snapshot — fans out to both `ensureBudgetExpensesSnapshotForYear`/`ensureBudgetIncomeSnapshotForYear` (see Year-scoped budget storage note above)
