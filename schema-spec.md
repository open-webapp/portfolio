...
CSV/paste import normalizes `date` to `YYYY-MM-DD` before the dedup key below is built — accepted input formats: ISO `YYYY-MM-DD`, `MM/DD/YYYY`/`M/D/YYYY`/`MM-DD-YYYY`/`M-D-YYYY` (slash/dash always read as MM/DD/YYYY, no ambiguity heuristics), and month-name formats (e.g. "Sep 17 2026"; a comma before the year, e.g. "Sep 17, 2026", breaks the row parser's comma-splitting and is not supported). Unparseable/unrecognized dates silently drop the row. See `product-behavior.md` for full parsing details.
