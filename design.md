...
App
  ...
  [view === 'dashboard']
    ...
    [tab === 'positions']
      ...
      PositionGroupOverlay    (group, accounts, dispatch, onClose)  — when a row is clicked; lists underlying positions sorted by account name. All editable fields use independent inline-edit UI with component-local state: click → input → Enter or blur commits via `UPDATE_POSITION` dispatch, Escape cancels/reverts (no dispatch). No new props added (still `{ group, accounts, dispatch, onClose }`). Editable cells: Symbol (`<input type="text">`, empty reverts silently), Account (dropdown button listing all accounts; selecting one dispatches `UPDATE_POSITION` with `patch: { accountId }`), Shares/AvgCost/Price/Taxes (`<input type="number">`; invalid/empty revert silently except Taxes empty → saves 0). Computed columns (Amount Invested, Market Value, G/L, G/L%) remain read-only text. Editing Symbol or Account changes position's `buildGroupKey()` result → position row disappears from currently-open overlay on next render (overlay itself stays open; no special wiring needed, natural re-render side effect).
...
4. "Reset app" (`onReset`) calls `clearPersistedApp()` then resets `gateShape` to `'absent'`, `sessionKey`/`sessionSalt` to `null`, and dispatches `__SET_STATE` with `initialState()`, returning to the set-password screen.
