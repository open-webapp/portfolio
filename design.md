...
- `App.tsx` also owns non-persisted `selectedScope` (init: newest transaction year, else `SPEND_ALL_YEARS` sentinel), shared by both Spend and Expenses and passed to `BudgetPage`; the year selector renders as a second row under the period tabs. `PeriodSegControl` takes an `expenseYears: string[]` prop (`App.tsx` computes it as `expenseBudgetYears(state.budgetExpenseAmountsByYear, new Date())`) and renders a plain year `<select>` with no "All" option when `period === 'expenses'`, vs. the All+year `<select>` (`availableYears={spendBudgetYears(state.budgetTransactions)}`) for Spend. `App.tsx`'s `handleScopeChange` (passed down as `onScopeChange`) sets `selectedScope` and dispatches `ENSURE_BUDGET_YEAR_SNAPSHOT` when the target year has no snapshot yet — shared by both tabs' scope changes.
- `BudgetPage.tsx` has two reconcile effects, both gated on `period`: one gated to `period === 'spend'` resets `selectedScope` to `SPEND_ALL_YEARS` when it's a concrete year not in `availableYears`; one gated to `period === 'expenses'` calls `onScopeChange(String(currentYear))` whenever `selectedScope === SPEND_ALL_YEARS` or isn't in `expenseBudgetYears(state.budgetExpenseAmountsByYear, now)` — this can fire on tab switch or after clearing a year's last expense amount removes it from the budgeted list.
...
- Income derivation: an active `Category` with an exact `Income` match resolves through `selectors.ts` into `BudgetPage` KPI tiles and savings-rate, gated by `effectiveCategoryId`/`excludedCategoryIdSet`. Spend selectors negate debit amounts for display. Legacy manual-income keys are dropped on hydration/import.
- Spend-scope selectors: `spendBudgetYears`, `SPEND_ALL_YEARS` sentinel, `spendTransactionsForScope`; `spendCardTotals` aggregates per exact-year snapshot (no nearest-year fallback); `budgetedSpend` is annualized per definition (`toPeriod(amount, definition.frequency, 'yearly')` summed across years in scope), matching `perCategoryBudgetActual`'s basis rather than using raw stored definition amounts. `ENSURE_BUDGET_YEAR_SNAPSHOT` dispatch creates a year's snapshot on demand. `availableBudgetYears` definition is unaffected by scope work. `spendScopeKind(scope, asOf): 'all' | 'current' | 'past' | 'future'` classifies a scope relative to `asOf`'s year. `yearElapsedFraction(year, asOf): number` — fraction of `year` elapsed as of `asOf` (inclusive-day, leap-year aware, clamped to `(0, 1]`).
- `savingsRateForScope(transactions, categories, definitions, scope: SpendScope): { pct: number; isPositive: boolean; income: number; spend: number } | null` — scope-aware savings rate alongside `savingsRateByYear` (unchanged, still used by `BudgetAnalytics.tsx`). Single-year scope: same income/spend as `savingsRateByYear` for that year. `SPEND_ALL_YEARS`: aggregates income/spend across all `spendBudgetYears` and computes one ratio over the totals (not an average of per-year rates). Returns `null` when total income <= 0.
...
- **KPI tiles** render in `BudgetPage.tsx`'s Spend branch: 4-tile `summary-cards`/`kpi-grid` (`kpi-spend`/`kpi-projected`/`kpi-savings`/`kpi-over-budget`, each `.card.card-compact.blueprint.elev-sm.kpi`). All keyed off `selectedScope` (year or `SPEND_ALL_YEARS`) plus `asOf`/`new Date()`, via `src/lib/selectors.ts`:
  - `spendPaceForScope(definitions, amountsByYear, transactions, categories, scope, asOf): { actual, budget, pctOfBudget, expectedByToday: number | null, isOnTrack }` — actual/budget from `spendCardTotals`; for the current-year scope only, `expectedByToday` prorates monthly-frequency budgets by `yearElapsedFraction` and caps yearly-frequency definitions' actual-to-date at their own budget; drives the `kpi-spend` tile's bar + pace marker.
  - `projectedSpendForScope(definitions, amountsByYear, transactions, categories, scope, asOfDate): { projectedTotal, budgetTotal, pctOver, isOverBudget } | null` — rewritten to be frequency-aware and to take `asOfDate`: current-year scope projects via `projectedSpendByCategory` (yearly-linked spend at actual, other spend extrapolated by elapsed-year fraction); past/future years return actual as-is; `SPEND_ALL_YEARS` returns `null` (no bounded period). Drives the `kpi-projected` tile.
  - `overBudgetCategoriesForScope(definitions, amountsByYear, transactions, categories, scope, asOfDate?): Array<{ categoryId, label, overageAmount, pctOver, status: 'over' | 'projected' }>` — delegates to `overBudgetCategories` for `status: 'over'` rows; with `asOfDate` and only for the current-year scope, appends `status: 'projected'` rows (via `projectedSpendByCategory`) for categories not already over whose projected spend exceeds annualized budget. Sort: all `'over'` rows (overage desc, label asc) then all `'projected'` rows (same ordering). Drives the `kpi-over-budget` tile.
  - `savingsRateForScope` (see above) drives the `kpi-savings` tile.
  - `BudgetExpensesTab.tsx` props: `{ state, dispatch, categories, categoryDispatch, selectedYear: string }` — `selectedYear` comes from `BudgetPage.tsx` (`selectedScope` coerced to a string; `SPEND_ALL_YEARS` never reaches Expenses thanks to its reconcile effect) and drives the Expenses table's 3 fixed year columns via `expenseTableColumnYears(selectedYear)` (`selectedYear-1`, `selectedYear`, `selectedYear+1`), not `expenseTableYears`'s union-of-years set.
  - Above the Expenses table, `data-testid="plan-stats-cards"` renders 4 read-only cards, each scoped to `selectedYear` and driven by a pure `selectors.ts` function: `plan-stat-spend` (`plannedSpendSummary`, planned spend), `plan-stat-savings` (`plannedSavingsRate`, planned savings rate), `plan-stat-frequency` (`plannedFrequencySplit`, monthly vs yearly split), `plan-stat-changes` (`planChanges`, plan changes vs `selectedYear-1`).
  - New `selectors.ts` signatures (Expenses planning):
    - `expenseBudgetYears(amountsByYear, now: Date): string[]` — budgeted years for the Expenses year select (amount-map keys + current year, descending).
    - `expenseTableColumnYears(selectedYear: string): [string, string, string]` — `[selectedYear-1, selectedYear, selectedYear+1]`, ascending.
    - `plannedSpendSummary(definitions, amountsByYear, categories, year): { annual, monthly, prior: { annual, delta, pct } | null }`.
    - `plannedSavingsRate(definitions, amountsByYear, categories, year): { income, spend, rate: number | null }`.
    - `plannedFrequencySplit(definitions, amountsByYear, categories, year): { monthlyTotal, yearlyTotal, setAsidePerMonth }`.
    - `planChanges(definitions, amountsByYear, categories, year): { increases: PlanChangeRow[], decreases: PlanChangeRow[], notCarriedOver: { count, names } }` (`PlanChangeRow = { expenseId, name, prior, current, delta, tag: 'new' | 'dropped' | null }`).
  - `expenseTableYears(transactions, amountsByYear, now)` (union-of-all-years) is now **CSV-export-only** — used solely by `expenseExport.ts`'s `buildExpenseCsv` (see Budget Expense CSV Export below); it is no longer used to render the on-screen Expenses table.

## Budget Expense CSV Export

- `expenseExport.ts`: `buildExpenseCsv(definitions, amountsByYear, transactions, categories, now)` builds an RFC4180-escaped CSV of expense rows, prefixing a leading apostrophe on values that would otherwise be read as a spreadsheet formula. Columns derive from `expenseTableYears(transactions, amountsByYear, now)` — the full union-of-years set (years from transaction dates and amount-map keys, plus the current calendar year) — independent of the Expenses table's 3 fixed display columns; rows sort by resolved category name then expense name. Transactions contribute years only; no transaction row/data is emitted.
- `BudgetExpensesTab.tsx` wires build + browser download; filename pattern `expenses-YYYY-MM-DD.csv`. Before building, a focused inline Name/Amount edit is committed and its projected post-edit state is used as the builder's input; an Escape-cancelled draft is excluded. Category/Frequency edits commit through their own `<select>` and are unaffected.
- `downloadCsvAsFile(csvText, filename)` (`lib/importExport.ts`) is the public browser Blob/anchor local-download utility (`text/csv;charset=utf-8`); it has no import, persistence, encryption, or Drive path.

## Budget Expense Paste Import

- `BudgetExpensesTab.tsx` local dialog state: visibility, `importYear` (initialized/reset to the current four-digit year), pasted text, and post-import result text. Derived on every render: `parseExpensePaste(importText)`, four-digit-year validity, case/trim-insensitive `Uncategorized` category lookup, and `planExpensePasteImport(...)` preview. Close/cancel/backdrop close resets all import-dialog local state; importing leaves the dialog open with the result message.
- Import flow: `parseExpensePaste` returns valid rows, errors, and total data-line count → `planExpensePasteImport` folds/plans valid rows against the selected year and resolved target category for preview counts → on import, the component finds the first case/trim-insensitive `Uncategorized` category or dispatches `ADD_CATEGORY` with that name and a generated id → the component dispatches one `IMPORT_EXPENSE_PASTE` action with the year, resolved category id, and valid parser rows → the reducer delegates to `importExpensePaste`, which creates definitions or updates selected-year amounts.
- Identity/scope: paste identity is `name.trim().toLowerCase()`. Existing-definition match is the first definition with that identity in the target `Uncategorized` category only. New definitions retain first pasted spelling, target `Uncategorized`, and use `monthly` frequency. Definitions are year-independent — `budgetExpenseAmountsByYear[year]` is the only amount map mutated by an import; other years remain unchanged.
...
- `TopBar` renders `header.top-bar` as a single-column CSS grid (inline `display: grid; gridTemplateColumns: '1fr'`, overriding the class's flex display) so the active portfolio's name (`<span class="nav-brand">{portfolioName}</span>`, required `portfolioName` prop, plain/non-interactive, `justifySelf: 'start'`, `whiteSpace: 'nowrap'`) and the optional Budget-only period control (wrapped in a `width: 100%` div) both occupy the same grid cell, overlaid rather than side-by-side. This keeps the period control centered on the *full* top-bar width independent of the name's length, and the name always renders in full on one line (no wrap, no truncation) — a very long name may visually overlap the tabs instead. It has no portfolio-switch button and no Sync button — those live in `RailNav`.
