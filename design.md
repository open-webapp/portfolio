...
src/
  ...
  components/
    ...
    DriveRestorePanel.tsx        # shared Drive connect/disconnect/restore UI: connect button, disconnect link, restore from Drive button, cross-password retry prompt, no-backup picker fallback; used by both PasswordGate restore tab and SettingsPage
...
2. `shape === 'absent'` → set-password screen with optional "Restore from Drive" tab-seg: offers "New Setup" (today's generate-key-and-set-password form) vs. "Restore from Drive" (temporary dummy key `generateSalt()` + `deriveKey(crypto.randomUUID(), dummySalt)` that guarantees the first `restoreBackup(dummyKey)` call always throws `DriveDecryptError`, reusing the cross-password-prompt code path verbatim with no special branching). On restore success (any of direct-key-match, cross-password-retry, or picker-fallback paths), calls `onUnlock(restoredKey, restoredSalt, restoredState)` just as the New Setup form does. Tab switching is free (no validation, state preserved per tab).
