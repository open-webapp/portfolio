// MIRROR OF src/lib/pastedTable.ts — edit both. src/lib/pastedTable.test.ts enforces parity via a shared CASES table.
(function () {
  /**
  ...
  function parseHtmlGrid(html) {
    ...
    const clamp = (n) => Math.max(1, Math.min(1000, n))
   ...
   */
  function tableToCsv(headersClipboard, valuesClipboard) {
  ...
  window.PastedTable = { tableToCsv: tableToCsv }
})();
