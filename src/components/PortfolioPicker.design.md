...
- Uses existing design-system classes only (`card blueprint elev-sm`, `seg`/`seg-opt`, `field`/`input`, `btn`/`btn-primary`/`btn-secondary`/`btn-ghost`/`btn-icon`, `tag tag-outline`); layout/spacing via inline `style` on wrapper `div`s. Picker column max width is 520px. Settings has a heading and import/download local-file icon actions (no Close button — tab switching is the exit); explanatory copy, summary/Manage, and `.hr` precede its separate Drive-sharing control. The "Manage" button navigates to `ManageCategoriesPage` (`navigateToCategories`) rather than opening an in-place dialog.
