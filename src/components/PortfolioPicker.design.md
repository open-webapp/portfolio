...
- **Top-level modes**: `Open`, `Create`, and `Google Drive` are radio-backed full-width `.seg` options below the centered `Ledger` / `Your portfolios` heading. The absolutely positioned Settings gear has a higher stacking level than the segments, so it remains visible and clickable over the selected Drive segment without constraining segment width. Changing a mode only changes which panel is rendered; it does not reset inactive panel state. The Drive card renders labeled `My portfolios` restore and `Shared portfolio` Picker/password sections sequentially, separated by `.hr`.
