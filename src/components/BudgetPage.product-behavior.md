...
- Tag filter combobox (type-to-search, multi-select) narrows to rows containing every selected tag (AND, case-insensitive); suggestions come from distinct tags in the show-excluded-scoped set (deduped, first casing kept, alpha-sorted). Typing never creates tags; no match shows an empty dropdown. Selection changes reset page + row selection; paging/sort/search/year changes clear the selection.
