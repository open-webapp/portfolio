...
- The Category Mapping tab groups portfolio-scoped expense-name + category -> substring mappings by category and expense definition (`<expense name> (<category>)`), with an `+ add substring` input per definition. Empty categories render no mapping rows. Mapping edits save on Enter; blank substrings are ignored. Deleting a mapping requires native confirmation.
## Spend Records
