...
- Expense definitions whose `categoryId` matches no global category render under `Unmatched expenses` (labelled `<expense name> (<categoryId>)`) with full substring add/edit/delete; mappings whose `spendExpenseId` matches no definition render under `Orphaned substrings` with delete.
