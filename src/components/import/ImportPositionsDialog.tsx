...
export interface ImportPositionsDialogProps {
  state: AppState
...
/**
 * ImportPositionsDialog: Multi-step dialog for importing positions from CSV.
 * 1. File picker
 * 2. Profile selection (or create new)
 * 3. Profile editor (if creating new)
 * 4. Review and apply mapping
 * 5. Hand off to Task 12 via pendingImport state
 */
export function ImportPositionsDialog({ state, dispatch }: ImportPositionsDialogProps) {
  ...
  const positionsProfiles = listProfilesForKind(state.mappingProfiles, 'positions')
  ...
  const handleSelectExistingProfile = useCallback((profileId: string) => {
    const profile = positionsProfiles.find((p) => p.id === profileId)
    if (profile) {
    ...
    <>
      ...
      {step === 'file-picker' && (
        <div>
          ...
          <p>Select a CSV file with position data</p>
          <input
            ref={fileInputRef}
            type="file"
            accept=".csv"
      ...
      {step === 'profile-select' && (
        <div>
          ...
          {positionsProfiles.length > 0 && (
            <div style={{ marginBottom: 'var(--space-4)' }}>
              <strong>Existing Profiles:</strong>
              <div style={{ marginTop: '8px' }}>
                {positionsProfiles.map((profile) => (
                  <button
                    key={profile.id}
