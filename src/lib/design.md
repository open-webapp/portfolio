...
- Clustering lives in `autoTag.ts` (zero imports; `state.ts` → `autoTag.ts` acyclic): adjacent-pair LCP with min-4-char trimmed threshold (`MIN_TAG_LENGTH`), whitespace never counts toward the minimum, singleton clusters untagged; a cut on affinity-lengthening without prefix-containment keeps divergent neighbors from diluting a coherent cluster's tag. Emitted tags pass `sanitizeTagToken` (mirrors `TagInput`: alphanumeric-only, `MAX_TAG_LENGTH` 10). Generic `applyAutoTags<T extends { description: string; tags?: string[] }>` reused by both triggers (manual passes `BudgetTransaction[]`, import passes raw rows).
...
- `BudgetPage.tsx` header left-group holds title + Auto-tag + Clear-all-tags buttons + shared transient ~4s feedback span; no tag-filter combobox.
- `CLEAR_BUDGET_TRANSACTION_TAGS` (no payload) → `clearBudgetTransactionTags` (`state.ts`): strips the `tags` key from every `budgetTransaction` (omitted, never `[]`), including manual tags. Component pre-computes the tagged count for the confirm dialog + feedback text.
