...
- Manual `Auto-tag records` pools ALL budget transactions across all years regardless of selected Spend-tab scope. Algorithm is adjacent-pair LCP clustering with min-4-char trimmed threshold; singleton clusters stay untagged; a divergent description sorting adjacent splits off instead of diluting the coherent cluster's tag (prefix-containment chains stay together). Emitted tags are sanitized like manual entry (alphanumeric-only, capped at 10 chars). Tags are additive, never replacing existing tags, capped at 5 per record.
