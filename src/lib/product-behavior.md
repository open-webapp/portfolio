...
- **Spurious version-drift auto-recover**: `RemoteChangedError` only means Drive's `version` counter moved past this device's baseline — which also happens on metadata-only server changes (a `viewedByMeTime` bump from this device's own restore-read, a sharing touch). When `getBackupFileStatus` reports a `lastRestoredAt` and the remote's content modified-time is **not** newer than it (`!(remoteModifiedTime > lastRestoredAt)`), the conflict is treated as drift: the app silently re-reads to adopt the current baseline and re-pushes local (`overwriteRemoteWithLocal`), then `alert('Synced to Drive')` — no dialog. Only a genuinely newer remote content time (or a missing `lastRestoredAt`, or a failed status probe) opens the dialog. If the silent re-push itself throws, the dialog opens as a fallback.
- **Dialog** (title "Drive backup changed"): shows remote-backup modified time (RFC3339) and local last-restored time (epoch ms from drive-sync) — each `—` when missing or unparseable, else `new Date(value).toLocaleString()`. Three buttons, all disabled while an action is pending; inline error region below the times.
...
- Budget flow expands vertically to keep small category labels readable; charts taller than 720px scroll within the card.
...
- Spend header left-group holds title + `Auto-tag records` button + `Clear all tags` button + transient feedback text; no tag-filter combobox. Feedback auto-clears after ~4s. A muted legend line under the toolbar reads "Gray = auto-tag, red = your tag".
- Each Spend record carries two parallel tag arrays: `tags` (user-only) and `autoTags` (system-only). Both omit-when-empty (key deleted, never `[]`). Combined cap: the merged unique count across both arrays (case-insensitive) caps at 5 per record — capacity change from 5 guaranteed user tags, since an auto tag can now occupy a slot.
- Read mode renders auto chips first (gray `tag-neutral`, `title="Auto-tag"` tooltip), then user chips (red `tag-outline`); an auto chip case-insensitively matching a user chip on the same record is hidden (your red casing wins). Auto-first order flips the old left-to-right habit for mixed rows.
- `Auto-tag records` pools ALL budget transactions across all years regardless of selected Spend-tab scope, and RECOMPUTES `autoTags` from scratch (overwrite, not additive): new clusters set it, stale tags vanish when a description changes, singleton/declustered rows get the key deleted. User `tags` are never touched. Algorithm is adjacent-pair LCP clustering with min-4-char trimmed threshold; a divergent description sorting adjacent splits off instead of diluting the coherent cluster's tag (prefix-containment chains stay together). Emitted tags are sanitized like manual entry (alphanumeric-only, capped at 10 chars).
- A cluster candidate matching the record's own user tags (case-insensitive) or refused by the combined 5-cap is skipped entirely — neither array evicts the other — leaving `autoTags` empty. Skips are excluded from the feedback count, which reports only records that actually gained an auto tag (`Tagged N record(s)`; removals not counted; `No new tags found` when zero).
...
- The tags cell editor shows gray auto chips read-only (no ×) plus an editable `TagInput` for user chips; typing a token matching an auto tag (case-insensitive) or exceeding the combined 5-cap is silently refused with no error copy (matches existing style).
- `Clear all tags` pools ALL budget transactions across all years regardless of selected Spend-tab scope, and opens a scope chooser dialog (radio, default User): User tags (count) / Auto-tags (count) / Both (count). Each scope confirms with its own affected count plus irreversible copy; a zero-count scope dispatches nothing (feedback only, e.g. `No auto-tags to clear`); Both dispatches the user-clear then the auto-clear sequentially. Cancel aborts silently. With zero tags anywhere the button shows feedback only, no dialog. Feedback shares the auto-tag transient text + ~4s auto-clear.
- Search matches both arrays (case-insensitive substring over merged `tags` + `autoTags`, so auto-only records are findable).
- CSV paste/upload 4th Tags column stays user-tags-only (no 5th column, no prefix syntax); OFX/QFX imports never set tags. JSON backup + Drive encrypted envelope carry both arrays automatically (whole-object shape). Old blobs load with all existing tags intact as user tags — missing `autoTags` means none, with no guessing and no migration writes.
